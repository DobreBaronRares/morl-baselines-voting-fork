"""Voting SARSA for single policy multi-objective reinforcement learning."""
...
class VoSARSA(MOPolicy, MOAgent):
    """Voting SARSA for single policy multi-objective reinforcement learning.

    Maintains one Q-table per objective, rely on a voting algorithm to choose the moves.
    ...
    def __init__(
        ...
        experiment_name: str = "VoSARSA",
        wandb_entity: Optional[str] = None,
        log: bool = True,
        seed: Optional[int] = None,
        parent_rng: Optional[np.random.Generator] = None,
    ):
        """Initializes the VoSARSA algorithm.

        Args:
            env: The environment to train on.
            id: The id of the policy.
            weights: The weights to use for the voting method.
            ...
            parent: The parent MPMOSARSA class in the case of multi-policy training.
        ...
        if model is not None:
            ...
            self.model = TabularModel(prioritize=None) if self.dyna else None

        self.log = log
        if self.log and parent_rng is None:
            self.setup_wandb(project_name, experiment_name+"__"+voting_method.__name__, wandb_entity)
    ...
    def eval(self, obs: np.array, w: Optional[np.ndarray] = None) -> int:
        """Greedily chooses best action using the voting method"""
        t_obs = tuple(obs)
        if t_obs not in self.q_table:
            return int(self.env.action_space.sample())
        return self.voting_method(self.q_table[t_obs], self.weights)
    ...
    def update(self):
        ...
        max_q = self.q_table[next_obs][self.action]
        ...
        if self.dyna:
            ...
            for _ in range(self.dyna_updates):
                ...
                max_q = self.q_table[next_s][self.action]
            ...
            wandb.log(
                {
                    ...
                    f"losses{self.idstr}/scalarized_td_error": np.dot(td_error, self.weights),
    ...
    def train(
        ...
        for _ in range(1, total_timesteps + 1):
            ...
            if eval_env is not None and self.log and self.global_step % eval_freq == 0:
                self.policy_eval(eval_env, scalarization=np.dot, weights=self.weights, log=self.log)
